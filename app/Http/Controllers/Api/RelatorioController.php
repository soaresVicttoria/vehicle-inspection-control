...
class RelatorioController extends Controller
{
    ...
    {
        $sql = "SELECT * FROM gabriel.veiculos ORDER BY modelo";
    ...
    {
        $sql = "SELECT p.nome_completo, v.id, v.modelo, v.placa, v.marca FROM gabriel.veiculos v JOIN gabriel.proprietarios p ON p.id = v.proprietario_id ORDER BY p.nome_completo";
    ...
    {
        $sql = "SELECT p.sexo, COUNT(v.id) as total_veiculos FROM gabriel.proprietarios p JOIN gabriel.veiculos v ON p.id = v.proprietario_id GROUP BY p.sexo ORDER BY total_veiculos DESC";
    ...
    {
        $sql = "SELECT marca, COUNT(*) as total_veiculos FROM gabriel.veiculos GROUP BY marca ORDER BY total_veiculos DESC";
    ...
    {
        $sql = "SELECT p.sexo, v.marca, COUNT(v.id) as total_veiculos FROM gabriel.veiculos v JOIN gabriel.proprietarios p ON p.id = v.proprietario_id GROUP BY v.marca, p.sexo ORDER BY total_veiculos DESC, v.marca, p.sexo";
    ...
    public function todosProprietarios()
    {
        $sql = "SELECT nome_completo, sexo, data_nascimento, EXTRACT(YEAR FROM AGE(CURRENT_DATE, data_nascimento)) as idade FROM gabriel.proprietarios ORDER BY nome_completo";
    ...
    public function proprietariosPorSexo()
    {
        $sql = "SELECT sexo, COUNT(*) as total_proprietarios, ROUND(AVG(EXTRACT(YEAR FROM AGE(CURRENT_DATE, data_nascimento))), 2) as idade_media FROM gabriel.proprietarios GROUP BY sexo ORDER BY sexo DESC";
    ...
    public function revisoesPorPeriodo()
    {
        $sql = "SELECT r.data_revisao, v.modelo, v.placa, v.marca FROM gabriel.revisoes r JOIN gabriel.veiculos v ON v.id = r.veiculo_id WHERE r.data_revisao BETWEEN '2023-03-01' AND '2024-06-01' ORDER BY r.data_revisao";
    ...
    {
        $sql = "SELECT v.marca, COUNT(r.id) as total_revisoes FROM gabriel.veiculos v JOIN gabriel.revisoes r ON r.veiculo_id = v.id GROUP BY v.marca ORDER BY total_revisoes DESC, v.marca";
    ...
    public function proprietariosComMaisRevisoes()
    {
        $sql = "SELECT p.nome_completo, p.sexo, COUNT(r.id) as total_revisoes FROM gabriel.proprietarios p JOIN gabriel.veiculos v ON v.proprietario_id = p.id JOIN gabriel.revisoes r ON r.veiculo_id = v.id GROUP BY p.nome_completo, p.sexo ORDER BY total_revisoes DESC";
